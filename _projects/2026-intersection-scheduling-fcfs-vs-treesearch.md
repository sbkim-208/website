...
Cooperative scheduling beating first-come-first-served is one of those things that sounds obviously true. I wanted to know if it actually is, and by how much, once I measured it instead of just assuming a smarter-sounding algorithm wins.

### 3. Goals

I wanted a fair fight: seven schedulers sharing the same request payload and the same hard constraints (never overlap the opposing axis, same-lane headway stays FIFO), benchmarked on the same scenarios. That included porting a real research algorithm (Xu et al.'s MCTS rollout heuristic) rather than a toy version of it, which meant a second goal I didn't originally plan for — making sure a literal port of someone else's algorithm actually preserved what made it work in their setup, not just its surface-level rule.

### 4. Process

My first version of the cost-benefit yielder (`algo_platoon`) used a threshold rule — yield whenever 2+ opposing vehicles were unassigned. I benchmarked it before trusting it, and it made average delay 59% worse than FCFS, because almost every request chose to yield and the yields chained into each other. I rewrote it to actually simulate both orderings ("I go first" vs. "the opposing platoon goes first") as real assignments and compare their total delay directly, picking whichever is lower. That version is the one in the benchmark below.

Implementing Xu et al.'s Algorithm 2 against a single conflict box, I found the literal version degenerates to plain FIFO — there's nothing left to exploit once there's only one region vehicles can conflict in. The paper's advantage comes from letting non-conflicting vehicles cross in parallel, which only exists if the conflict zone is subdivided, so I split the intersection into 4 conflict subzones (NE/NW/SE/SW) so a vehicle occupies 2 subzones in sequence instead of the whole box at once.

Even after that fix, early benchmarks under my original protocol — assign a slot immediately when requested — showed MCTS basically tying with the much cheaper `algo_platoon` heuristic, even at 60 vehicles. Instead of concluding MCTS just wasn't worth it, I went back to the paper's actual protocol, which re-plans every vehicle in the control zone every 2 seconds instead of committing a slot the instant it's asked for. I implemented `replan()` to match it.

### 5. Result

**First**, the yielding rewrite flipped the result completely — same intuition (yield when it helps the group), opposite outcome, because the second version measures the actual delay instead of guessing at a proxy for it:
...
**Third**, the protocol fix mattered just as much as the algorithm itself. Once requests were evaluated together (`replan()` every 2s) instead of one at a time, MCTS's advantage over the cheaper heuristics showed up — the gap was real, my protocol had just been hiding it. Full benchmark, same scenario, average delay improvement over FCFS:
...
**Fourth**, a finding that didn't come from fixing a bug — priority policy and P2P visibility turned out to be independent axes. Priority works fine in a fully centralized scheduler, since it's just a scoring change, not a structural one. P2P's partial visibility is a real safety-vs-decentralization trade-off though, and I don't think it's something you can design away — only measure. The comm-range slider in the sim trades safety violations against decentralization directly, and that trade-off doesn't go away no matter which scheduler sits behind it.

### Algorithms and datasets

Seven schedulers share the same request payload (vehicle state, confirmed grants, per-lane last slot, full vehicle snapshot, in-range neighbors):
